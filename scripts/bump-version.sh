...
# Requires: jq
#
# Non-JSON files (e.g. scripts/install.sh, scripts/install.ps1) that embed
# a version literal are NOT bumped automatically. The --audit subcommand
# flags them so you can update them manually.
...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq not found. Install it: brew install jq (macOS) or apt install jq (Linux)" >&2
  exit 1
fi
...
# Write a dotted field path in a JSON file, preserving formatting where jq allows.
write_json_field() {
...
cmd_audit() {
  ...
  if [[ "$found_undeclared" -eq 0 ]]; then
    ...
    echo "If they should be skipped (e.g. CHANGELOG entries), add them to audit.exclude."
    echo "For shell/powershell install scripts that embed the version as a string literal,"
    echo "update them manually with sed after running the bump:"
    echo "  sed -i '' 's/\"version\": \"$current_version\"/\"version\": \"NEW\"/' scripts/install.sh scripts/install.ps1"
  fi
}
